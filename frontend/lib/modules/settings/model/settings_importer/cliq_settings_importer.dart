...
class CliqSettingsImporter extends AbstractSettingsImporter {
  ...
  Future<bool> canParse(String path, String content, {String? password}) async {
    if (content.length % 4 == 0 &&
      ...
      content = utf8.decode(await _decodeAndDecrypt(content, password: password));
    } else {
  ...
  @override
  Future<AppSettings?> tryParse(String path, String content, {String? password}) async {
  ...
  Future<Uint8List> _decodeAndDecrypt(String content, {String? password}) async {
    ...
    if (PasswordCipher.instance.isEncrypted(decoded)) {
      ...
      try {
        decoded = await PasswordCipher.instance.decrypt(decoded, utf8.encode(password));
      } catch (e) {
