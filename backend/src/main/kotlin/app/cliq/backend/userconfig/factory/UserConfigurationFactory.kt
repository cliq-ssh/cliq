...
@Service
class
UserConfigurationFactory(private val clock: Clock) {
    fun createFromParams(configurationParams: ConfigurationParams, user: User): UserConfiguration =
        create(configurationParams.configuration, user)
    ...
    fun create(encryptedConfig: String, user: User): UserConfiguration = UserConfiguration(
        user = user,
