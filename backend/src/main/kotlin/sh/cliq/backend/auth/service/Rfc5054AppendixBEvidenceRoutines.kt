...
object Rfc5054AppendixBClientEvidenceRoutine : ClientEvidenceRoutine {
    override fun computeClientEvidence(
        config: SRP6CryptoParams,
        ctx: SRP6ClientEvidenceContext,
    ): BigInteger {
        val digest = MessageDigest.getInstance(config.H)
        return computeRfc5054AppendixBM1(
            digest, config.N, config.g, ctx.userID, ctx.s, ctx.A, ctx.B, ctx.S,
        )
    }
}

object Rfc5054AppendixBServerEvidenceRoutine : ServerEvidenceRoutine {
    override fun computeServerEvidence(
        config: SRP6CryptoParams,
        ctx: SRP6ServerEvidenceContext,
    ): BigInteger {
        val digest = MessageDigest.getInstance(config.H)
